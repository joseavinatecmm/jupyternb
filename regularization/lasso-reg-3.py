...
# 4. Aplicar Lasso Regression para reducir el COD al 80%
# Ajustar el parámetro alpha para lograr un COD del 80% en prueba
target_test_r2 = 0.80
alpha = 0.5  # Empezamos con un valor de alpha
tolerance = 0.01  # Tolerancia para alcanzar el COD deseado
...
while True:
    ...
    if abs(test_r2_lasso - target_test_r2) < tolerance:
        break
    elif test_r2_lasso > target_test_r2:
        alpha *= 1.1  # Aumentar alpha para reducir el COD
    else:
        alpha *= 0.9  # Disminuir alpha para aumentar el COD

# Predecir en los conjuntos de entrenamiento y prueba con Lasso
...
print(f"\nAlpha utilizado en Lasso: {alpha:.4f}")
print(f"COD en entrenamiento (Lasso Regression): {train_r2_lasso:.4f}")
...
plt.title("Lasso Regression para Reducir el COD al 80%")
plt.xlabel("X")
plt.ylabel("y")
plt.legend()
plt.show()
